...
public class MemberService {
	...
	public String signUp() {
		...
		if( memberPw.equals(memberPw2)) {
		 ...
		 //***** 밑에서 리턴받은 인덱스 번호만큼 위에 애들을 넣어주겠다
	...
	public int emptyIndex() {
		...
		// for문을 수해했지만 return이 되지 않은 경우 해당 위치 코드가 수행!
		//-> for문에서 return되지 않았다는 뜻은 배열에 빈칸이 없다는 뜻
		// 그러면 -1을 반환해라
		return -1;
	}
	...
	// 1) memberArr 배열 내 요소를 순서대로 접근하여 null이 아닌지 확인
		for(int i = 0; i < memberArr.length; i++) {// 회원 정보가 잇을 경우
			if(memberArr[i] != null) {
				// 2) 회원정보 (memberArr[i])의 아이디, 비밀번호와
				// 입력받은 아이디, 비밀번호가 같은 건지 확인
			...
			if( loginMember == null) { // 로그인 실패
				...
				return loginMember.getMemberName() + "님 화녕합니다";}
	}
	...
	// 회원 정보수정 메서드
		public int updateMember() {
			...
			sc.nextLine(); // int 다음에는 한번씩 넣어주기 입력버퍼 정리용
			
			System.out.print("수정할 지역 입력 : ");
			String inputRegion = sc.next();  // 지역변수
			
			
			
			
			// 3) 비밀번호를 입력받아서 로그인한 회워늬 비밀번호와 일치한지 확인
			System.out.print("비밀번호 입력 : ");
			String inputPw = sc.next();
			
			if( inputPw.equals(loginMember.getMemberPw())) {
				// 4) 비밀번호가 일치하는 경우
				//	로그인한 회원의 이름, 나이, 지역 저오를 입력받은 ㄱ밧으로 변경 후 1 반환
				
				loginMember.setMemberName(inputName);
				loginMember.setMemberAge(inputAge);
				loginMember.setRegion(inputRegion);
				
				// loginMember = inputName;  => 이거는 프라이빗이라서 이쪽에서 쓸 수가 없어서
				// 게터세터를 통한 위에꺼를 사용
			
				return 1;
			} else {// 5) 비밀번호가 다를 경우에 0 반환
				return 0;
			}
		}
	
		//회원 검색(지역) 매서드
		public void searchRegion() { //가지고 돌아갈 값없음 걍 실행만 하고 끝낼거
			System.out.println("\n********** 회원 검색(지역) **********");
			...
			for(int i = 0; i < memberArr.length; i++) {
				...
				if(memberArr[i] == null) break;  // 조건안에 실행 되는 구문이라면 중괄호 안적어도 됨
			
			// 3) memberArr[i] 요소에 저장된 지역(getRegion())이
			//	입력받으 지역과(inputRegion)과 같을 경우, 회원, 아이디, 이름 출력
			if(memberArr[i].getRegion().equals(inputRegion)) {
				System.out.printf("아이디 : %s, 이름 : %n",
						memberArr[i].getMemberId(),
						memberArr[i].getMemberName() ); // 바로 프린트 쓰면서 리턴해서 가지고 갈 값이 없음 그래서 void
